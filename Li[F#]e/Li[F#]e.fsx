// Li[F#]e (Game of life) by GMD, 09-04-2012
// Regole:
...
type TheWorld(ww : int, wh : int, c:int, s:int, i:int) as x=
    ...
    member x.Calculate() =
        ...
        for i in 0 .. height-1 do
            for j in 0 .. width-1 do
                if matrix.[i,j] = 1 then matrix.[i,j] <- 3
                if matrix.[i,j] = 2 then matrix.[i,j] <- 0
        ...
        for i in 0 .. height-1 do
            ...
            for j in 0 .. width-1 do
                ...
                if matrix.[i,j] = 3 && (count < 2 || count > 3) then matrix.[i,j] <- 2
                if matrix.[i,j] = 0 && count = 3 then matrix.[i,j] <- 1
...
//width and height
let w, h = 200 , 100
// cellsize and spacing 
let c, s = 8, 2
//refresh interval
let i = 1000
/////////////////////////////////////

let f = new Form(Text="Li[F#]e || PAUSE", TopMost=true)
...
//f.Icon <- new Icon(@"D:/life.png")

theworld.KeyDown.Add(fun e ->
    match e.KeyData with
    | Keys.Space -> 
        let status = theworld.StartStop()
        let title = sprintf "Li[F#]e  %s" status
        f.Text <- title 
    | Keys.C ->
        theworld.Clear()
        f.Text <- "Li[F#]e  X CLEARED"
    | Keys.W ->
        theworld.SpeedUp()
    | Keys.S ->
        theworld.SpeedDown()
